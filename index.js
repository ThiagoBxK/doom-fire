...
function renderFire() {
  ...
  const createCell = (intensity, columnIndex, cellIndex) => {
    ...
    context.fillStyle = `rgb(${fire.pallet[intensity >= 0 ? intensity : 0]})`;
    
    context.fillRect(
      columnIndex * pixelWidth, cellIndex * pixelHeight, 
      pixelWidth, pixelHeight
    );
  };
  
  firePixels.columns.forEach((column, columnIndex) => {
    column.forEach((intensity, cellIndex) => createCell(intensity, columnIndex, cellIndex));
  });
}
...
function firePropagation() {
  const newFirePixels = firePixels.columns.map((column) => {
    return column.map((currentCell, index) => {
      ...
      currentCell = currentCell >= 0 ? (belowCell !== undefined ? belowCell : 36 + decay) - decay : 0;
...
function createFireStructure() {
  ...
  firePixels = {
    columns: Array(fire.height).fill(0).map(() => Array(fire.width).fill(0))
  };
